...
async function _tubeStatus(req, res, next) {
  this.logger.trace(`${this._traceStack()} - Tube status API called`);
  ...
  try {
    this.logger.debug(`${this._traceStack()} - Getting data from TFL`);
    const url = `https://api.tfl.gov.uk/Line/${line}?${TFLAPIKey}`;
    this.logger.debug(`${this._traceStack()} - ${url}`);
...
async function _nextTube(req, res, next) {
  this.logger.trace(`${this._traceStack()} - Next tube API called`);
  ...
  try {
    ...
    if (!helper.isEmptyObject(apiData)) {
      ...
      this.logger.debug(`${this._traceStack()} - Get departure station`);
...
async function _busStatus(req, res, next) {
  this.logger.trace(`${this._traceStack()} - Bus Status API called`);
...
async function _nextbus(req, res, next) {
  this.logger.trace(`${this._traceStack()} - Next Bus API called`);
  ...
  switch (route) {
    case '380':
      this.logger.debug(`${this._traceStack()} - Using Bus no. 380`);
      url = `https://api.tfl.gov.uk/StopPoint/490013012S/Arrivals?mode=bus&line=380&${TFLAPIKey}`;
      break;
    case '486':
      this.logger.debug(`${this._traceStack()} - Using Bus no. 486`);
      ...
      break;
    case '161':
      this.logger.debug(`${this._traceStack()} - Using Bus no. 161`);
  ...
  try {
    ...
    this.logger.debug(`${this._traceStack()} - Get data from TFL`);
    this.logger.debug(`${this._traceStack()} - ${url}`);
    const apiData = await this._callAPIServiceGet.call(this, url);
    if (apiData instanceof Error) {
      this.logger.error(`${this._traceStack()} - ${apiData.message}`);
...
async function _nextTrain(req, res, next) {
  this.logger.trace(`${this._traceStack()} - Next train API called`);
  ...
  try {
    this.logger.debug(`${this._traceStack()} - Get data from API`);
    const apiData = await this._callAPIServiceGet.call(this, url);
    ...
    this.logger.debug(`${this._traceStack()} - Sort by departure time`);
    trainData = cleanData.sort(helper.getSortOrder('aimed_departure_time'));

    this.logger.debug(`${this._traceStack()} - Construct JSON`);
    let returnJSON = [];
    let trainStations;
    let journey;
    ...
    for (let index = 0; index < maxJourneyCounter; index += 1) {
      ...
      this.logger.debug(`${this._traceStack()} - Check for cancelled train`);
      if (
        trainData[index].status.toLowerCase() === 'it is currently off route' ||
        trainData[index].status.toLowerCase() === 'cancelled'
      ) {
        this.logger.debug(`${this._traceStack()} - Found distrupted train`);
        disruptions = true;
        anyDisruptions = true;
      }

      if (!disruptionsOnly) {
        this.logger.debug(`${this._traceStack()} - Get stops info`);
        ...
        if (apiData instanceof Error) {
          ...
          this.logger.debug(`${this._traceStack()} - Work out duration`);
          duration = helper.timeDiff(departureTime, arrivalTime);

          this.logger.debug(`${this._traceStack()} - Construct journey JSON`);
          journey = {
            mode,
            line,
            disruptions,
