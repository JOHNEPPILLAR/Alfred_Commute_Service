...
async function _getCommuteStatus(req, res, next) {
  this.logger.debug(`${this._traceStack()} - Get commute status API called`);

  let anyDisruptions = false;

  this.logger.trace(`${this._traceStack()} - Checking for params`);
  let apiData;
  try {
    this.logger.trace(`${this._traceStack()} - Get train status`);
...
function _returnCommuteError(message, req, res, next) {
  ...
  this.logger.trace(`${this._traceStack()} - ${message}`);
  journeys.push({ legs });
...
async function _getCommute(req, res, next) {
  this.logger.debug(`${this._traceStack()} - Get commute API called`);
  ...
  if (atHome) {
    ...
    this.logger.trace(`${this._traceStack()} - Checking train and tube status`);
    ...
    if (trainData.anyDisruptions === 'false') {
      ...
      const legs = [];
      this.logger.trace(`${this._traceStack()} - Add train leg`);
      legs.push(apiData[0]);
      ...
      this.logger.trace(`${this._traceStack()} - Add walking leg`);
      legs.push(walkLeg);
      ...
      this.logger.trace(`${this._traceStack()} - Add bus leg`);
      legs.push(busLeg);
  ...
  this.logger.trace(`${this._traceStack()} - Find out if at work location`);
  try {
  ...
  if (atJPWork) {
    ...
    // Add train leg
    this.logger.trace(`${this._traceStack()} - Check train status`);
  ...
  if (!atHome && !atJPWork) {
    this.logger.trace(`${this._traceStack()} - Add not at home or work`);
    if (typeof res !== 'undefined' && res !== null) {
      _returnCommuteError(
        'Unable to calculate commute due to starting location',
        req,
