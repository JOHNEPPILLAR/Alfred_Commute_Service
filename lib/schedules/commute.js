...
async function checkDistruptions() {
  ...
  try {
    this.logger.debug(`${this._traceStack()} - Getting commute status`);
  ...
  if (commuteDistruption) {
    this.logger.debug(`${this._traceStack()} - There are commute distruptions`);
    if (!global.commuteDistruptions) {
      ...
      this.logger.debug(`${this._traceStack()} - Notification already sent`);
    }
    ...
    if (global.commuteDistruptions) {
      ...
      this.logger.debug(`${this._traceStack()} - ${messageToSend}`);
      this._sendPushNotification.call(this, messageToSend);
    }
...
async function checkForBankHolidayWeekend() {
  ...
  if (isJPAtHome instanceof Error) {
    this.logger.debug(`${this._traceStack()} - ${isJPAtHome.message}`);
    return;
  }
...
async function setupSchedules() {
  ...
  try {
    ...
    this.logger.debug(`${this._traceStack()} - Connect to data store`);
    const dbConnection = await this._connectToDB('commute');
    this.logger.debug(`${this._traceStack()} - Get commute schedule settings`);
    ...
    this.logger.debug(
      `${this._traceStack()} - Add commute schedule to base class schedules`,
    );
    this._addSchedule(date, 'Commute check', checkForBankHolidayWeekend);
  } catch (err) {
    this.logger.error(`${this._traceStack()} - ${err.message}`);
  }
}
